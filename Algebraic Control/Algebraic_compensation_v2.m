%--------------------------------------------------------------------------
% Algebraic Sloshing Compensation (Comau SmartSix robot) - Scara Motions
%
...
toolOffset  = [0 0 0.056]' + [0 0 0.01]'; % FT sensor + tray thick
...
if ~comp
    fig_name = strcat('Data/100325/',motion_type,'_', num2str(dist),'m_',num2str(Te),'s_',num2str(rad2deg(rot)),'deg','_noComp');
else
    fig_name = strcat('Data/100325/',motion_type,'_', num2str(dist),'m_',num2str(Te),'s_',num2str(rad2deg(rot)),'deg');
end
if rot ==0 
...
if comp
    for i=1:n
        if ~isempty(th)
            phi(i) = -th(i) + pi + atan2(rEddy(i),rEddx(i));
        else
            phi(i) = pi + atan2(rEddy(i),rEddx(i));
        end
        theta(i) = atan2(sqrt(rEddy(i)^2+rEddx(i)^2),(g+rEddz(i)));
        % theta_dot(i) = ( (g + rEddz(i)) * (rEddy(i) * rEdddy(i) + rEddx(i) * rEdddx(i)) / sqrt(rEddy(i)^2 + rEddx(i)^2) - sqrt(rEddy(i)^2 + rEddx(i)^2) * rEdddz(i) ) / ( (g + rEddz(i))^2 * (1 + (rEddy(i)^2 + rEddx(i)^2) / (g + rEddz(i))^2) );
    end
    
    for i=2:n
        theta_dot(i) = (theta(i)-theta(i-1))/(1/freq);
        phi_dot(i) = (phi(i)-phi(i-1))/(1/freq);
        theta_ddot(i) = (theta_dot(i)-theta_dot(i-1))/(1/freq);
        phi_ddot(i) = (phi_dot(i)-phi_dot(i-1))/(1/freq);
    end
end

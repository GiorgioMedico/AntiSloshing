function [Fr, Fz] = PIndexPlus(numSloshMasses,h,R,EOMtype,N,erased_path,Te,str,k_eps,k_sigma,objFuncType)
...
% for i = 1:N  % <-- Parallel for loop
    ...
    if strcmp(erased_path(i),"Tilt")
        ...
        for j = 1:numSloshMasses
            ...
            sNLt = RK4_odeTiltP_plus_res(wn(j),zitan(j),str(i).rEddy,-str(i).rEddx,str(i).rEddz,str(i).phiOde,str(i).phidOde,str(i).phiddOde,dn(j),g,EOMtype,str(i).time);
            tNLt = str(i).time;
        ...
        for j = 1:numSloshMasses
            ...
            tic
            sNLk = RK4_odeSchoenP_plus_res(wn(j),zitan(j),str(i).rEddy,-str(i).rEddx,str(i).rEddz,str(i).phiOde,str(i).phidOde,str(i).phiddOde,g,EOMtype,str(i).time);
            toc
            tNLk = str(i).time;
